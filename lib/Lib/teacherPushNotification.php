...
class teacherPushNotification {
    ...
    public $teacherPlatformApplicationArn = array(
        'APNS' => 'arn:aws:sns:ap-northeast-1:611762151909:app/APNS/nativecamp-teacher-ios-production',
        'APNS_SANDBOX' => 'arn:aws:sns:ap-northeast-1:611762151909:app/APNS_SANDBOX/nativecamp-teacher-ios-development',
        'GCM' => 'arn:aws:sns:ap-northeast-1:611762151909:app/GCM/nativecamp-teacher-android-production'
    );

    public $teacherSettings = array(
        'APNS_SANDBOX' => array(
            'platform_name' => 'nativecamp-teacher-ios-development',
            'cert_file_name' => 'dev/teacher-apns-dev-cert.pem',
            'key_file_name' => 'dev/teacher-apns-dev-key.pem'
        ),
        'APNS' => array(
            'platform_name' => 'nativecamp-teacher-ios-production',
            'cert_file_name' => 'live/teacher-apns-cert.pem',
            'key_file_name' => 'live/teacher-apns-key.pem'
        ),
        'GCM' => array(
            'platform_name' => 'nativecamp-teacher-android-production',
            'server_api_key' => 'AAAAHvpVkOY:APA91bFMZoTV-oy3R7lxcn-asHJSMr3ppnAi8Q7zPAbxR_VVGmaIXv2wanjxAPlPeVqhKCKbK4LP0x22HEc9_Td3Mf7-R-iI3PiQE8KhSRNqjLh0UESH8bbkxpdIy2_BnzP7fZlZpWBX'
        )
    );
    ...
    public function __log_error($method = null, $message = null) {
        CakeLog::write('error', "[TEACHER_PUSH_NOTIFICATION] ". $method ."|| ".$message);
    }

    public function __log_debug($message = null) {
        CakeLog::write('debug', "[TEACHER_PUSH_NOTIFICATION] ". $message);
    }

    public function createPlatformArn($platform = null){
        if ($this->sns && !is_null($platform)) {
            $attributes = array();
            $platformSettings = $this->teacherSettings;
            ...
            if ($platform == 'APNS_SANDBOX' || $platform == 'APNS') {
                ...
                $apnsPK = ROOT . DS . "lib" . DS . "Certs" . DS . "teacher" . DS . $pKeyFileName;
                $apnsCert = ROOT . DS . "lib" . DS . "Certs" . DS . "teacher" . DS . $certFileName;
    ...
    public function deletePlatformApplication($platform = null){
        ...
        $platformApplicationArnSet = $this->teacherPlatformApplicationArn;
        ...
        if (is_null($endpointArn) || !$this->sns) {
            ...
            try {
                ...
                if ($result && isset($result['Attributes']['Enabled'])) {
                    ...
                    $response['enabled'] = filter_var($result['Attributes']['Enabled'], FILTER_VALIDATE_BOOLEAN);
                }
            } catch (SnsException $e) {
                //catch error in attempt to delete endpoint, log error
                $response['error'] = $e->getMessage();
    ...
    public function setupPayload($platform = null, $type = null, $custom = array()) {
        ...
        if (is_null($platform) || !is_array($custom)) {
            $error = 'Invalid arguments passed.';
            $this->__log_error(__METHOD__, "Error: ". $error ." || Arguments: ". json_encode(func_get_args()));
            return $payload;
        }
        ...
        if ($type == Configure::read('teacher_push_notification_type.booked_reservation')) {
            $message = 'Your class slot has been reserved! - NativeCamp.';
        } else if ($type == Configure::read('teacher_push_notification_type.cancelled_reservation')) {
            $message = 'Your booked class has been cancelled. - NativeCamp.';
        } else if ($type == Configure::read('teacher_push_notification_type.upcoming_reservation')) {
            $message = 'Your booked class will start in 5 minutes. - NativeCamp.';
    ...
    /**
     * common function to send push notification according to type for teacher side
     */
    public static function sendPushNotif($type = null, $deviceType = null, $endpoint = null, $custom = array()) {
        $result = false;
        $pn = new teacherPushNotification();
