...
class UsersNationalityListController extends AppController {
	public $uses = array(
		'CountryCode', 
		'User',
		'Nationality'
	...
	public function index() {
		...
		if (isset($data['users_api_token']) && !is_string($data['users_api_token'])) {
			...
			$nationalityList = $this->Nationality->getNationality();
			$response = array('nationality_list' => $nationalityList);
		}
		return json_encode($response);
	}
}